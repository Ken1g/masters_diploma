...
class visualizer(object):
    ...
    def loss_plot(self, loss, val_loss, machine):
        import matplotlib.pyplot as plt
        ...
        ax.set_yticks(np.arange(9, 14., 0.5))
        ax.set_ylim([9, 14])
        ax.set_xlim([0, 100])
        ax.grid()
        ax.plot(loss)
        ax.plot(val_loss)
        ax.set_title("Model loss ({})".format(machine))
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
...
##################################3
        rtype = ''
        if rtype == 'drop_freq':
            reverberation_params = {
                'type': 'drop_freq'
            }
            noisifier = DropFreq()
        else:
            reverberation_params = None
            noisifier = None
####################################
#####################################3
######################################33
...
#####################################
        vectors = com.file_to_vectors(file_list[idx],
                                                ...
                                                power=power,
                                                reverberation_params=reverberation_params,
                                                noisifier=noisifier)
...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        visualizer.loss_plot(history.history["loss"], history.history["val_loss"], machine_type)
