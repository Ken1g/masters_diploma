...
class visualizer(object):
    ...
    def roc_auc(self, y_test_source, preds_source, y_test_target, preds_target, name):
        import matplotlib.pyplot as plt
        ...
        fpr, tpr, threshold = metrics.roc_curve(y_test_source, preds_source)
        ...
        ax.set_title('ROC {}'.format(name))
        ax.plot(fpr, tpr, label = 'AUC (source) = %0.3f' % roc_auc)

        fpr, tpr, threshold = metrics.roc_curve(y_test_target, preds_target)
        roc_auc = metrics.auc(fpr, tpr)
        # ax = self.fig.add_subplot(1, 1, 1)
        # ax.cla()
        # ax.set_title('Receiver Operating Characteristic')
        ax.plot(fpr, tpr, label = 'AUC (target) = %0.3f' % roc_auc)
    ...
    def save_figure(self, name):
        ...
        return : None
        """
        self.plt.savefig('{}/{}'.format('roc_auc', name))
...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        if mode:
            ...
            csv_lines.append(["section", "AUC (source)", "AUC (target)"])
            performance = []
        ...
        for section_name in section_names:
            ...
            if mode:
                y_test_source = [y_true[idx] for idx in range(len(y_true)) if domain_list[idx]=="source"]
                y_test_target = [y_true[idx] for idx in range(len(y_true)) if domain_list[idx]=="target"]
                y_score_source = [y_pred[idx] for idx in range(len(y_pred)) if domain_list[idx]=="source"]
                y_score_target = [y_pred[idx] for idx in range(len(y_pred)) if domain_list[idx]=="target"]
                auc_source = metrics.roc_auc_score(y_test_source, y_score_source)
                auc_target = metrics.roc_auc_score(y_test_target, y_score_target)
                fpr_source, tpr_source, threshold_source = metrics.roc_curve(y_test_source, y_score_source)
                roc_auc_source = metrics.auc(fpr_source, tpr_source)
                fpr_target, tpr_target, threshold_target = metrics.roc_curve(y_test_target, y_score_target)
                roc_auc_target = metrics.auc(fpr_target, tpr_target)
                ...
                csv_lines.append([section_name.split("_", 1)[1],
                                auc_source, auc_target])

                visualizer.roc_auc(y_test_source, y_score_source, y_test_target, y_score_target, '{}_{}'.format(machine_type, section_name))
                visualizer.save_figure('{}_{}'.format(machine_type, section_name))

                # visualizer.roc_auc(y_test_target, y_score_target)
                # visualizer.save_figure('{}_{}_{}'.format(machine_type, section_name, 'target'))
    ...
    if mode:
        csv_lines.append(["", "AUC (source)", "AUC (target)"])      
